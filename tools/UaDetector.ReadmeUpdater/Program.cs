...
var mobileApps = MobileAppParser.MobileApps
    .Select(app => app.Name)
...
var modifiedReadme = originalReadme
    ...
    .ReplaceBetweenMarkers("PERSONAL-INFORMATION-MANAGERS", PimParser.PersonalInformationManagers.Select(x => x.Name))
    .ReplaceBetweenMarkers("MEDIA-PLAYERS", MediaPlayerParser.MediaPlayers.Select(x => x.Name))
...
{
    modifiedReadme = modifiedReadme.ReplaceBetweenMarkers("LAST-UPDATED", DateTime.Today.ToString("yyyy-MM-dd"));
}
